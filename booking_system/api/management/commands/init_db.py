...
class Command(BaseCommand):
    help = "Initialize the database if it does not exist (supports MySQL)."

    def add_arguments(self, parser):
        # Root credentials are read from environment variables
        # (`MYSQL_ROOT_USER`, `MYSQL_ROOT_PASSWORD`).
        # The command no longer accepts CLI flags for root credentials.
        return
    ...
    def handle(self, *args, **options):  # noqa: C901
        ...
        if "sqlite3" in engine:
            self.stdout.write(self.style.SUCCESS(f"SQLite database used at {name}; file will be created by Django migrations."))
            return

        if "mysql" in engine:
            if MySQLdb is None:
                self.stdout.write(self.style.ERROR("mysqlclient (MySQLdb) is not installed in this Python environment. Please install it to run this command."))
                return
